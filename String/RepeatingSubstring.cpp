...
struct SuffixAutomaton {
    ...
    // Longest repeated substring (occurs >= 2 times)
    string longestRepeated(const string &S) const {
        int bestLen = 0, endPos = -1;
        for (int v = 1; v < (int)st.size(); v++) {
            if (occ[v] >= 2 && st[v].len > bestLen) {
                bestLen = st[v].len;
                endPos  = st[v].first_pos;
            }
        }
        if (bestLen == 0) return "";
        return S.substr(endPos - bestLen + 1, bestLen);
    }
};
...
int main() {
    ...
    string ans = sam.longestRepeated(S);
    if(ans.empty()) cout<<"-1\n"; else cout<<ans<<"\n";
